...
<h1 className='font-semibold text-[60px] max-xl:text-[35px]  max-md:mt-[10px] text-nowrap'>{t("our")}</h1>
            ...
            <div className={`products relative grid grid-cols-4 max-2xl:grid-cols-3 gap-[40px] w-full mt-[60px] justify-start justify-items-center  max-xl:grid-cols-2 max- max-md:grid-cols-1`}>
            {products.slice(0,8).map((e:any) => (
 <Product element={e} Title={e.Title} category={e.category} count={e?.count} imagesize="w-full" Description={e.description} Rate={5} price={e.price} discount={e.discount} thumb={e.image} id={e.id} key={e.id} />
))}
